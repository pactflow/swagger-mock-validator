import { g as getDefaultExportFromCjs, _ as _asyncToGenerator, a as _regeneratorRuntime, b as _ } from './swagger-mock-validator-63cb78d9.js';
...
import { p as packageJson, S as SwaggerMockValidatorFactory } from './swagger-mock-validator-factory-aad0297c.js';
import 'assert';
import 'stream';
import 'querystring';
import 'url';
...
program.version(packageJson.version).arguments('<swagger> <mock>').option('-p, --provider [string]', 'The name of the provider in the pact broker').option('-t, --tag [string]', 'The tag to filter pacts retrieved from the pact broker').option('-u, --user [USERNAME:PASSWORD]', 'The basic auth username and password to access the pact broker').option('-a, --analyticsUrl [string]', 'The url to send analytics events to as a http post').option('-o, --outputDepth [integer]', 'Specifies the number of times to recurse ' + 'while formatting the output objects. ' + 'This is useful in case of large complicated objects or schemas.', parseInt, 4).option('-A, --additionalPropertiesInResponse [boolean]', 'allow additional properties in response bodies, default false').option('-R, --requiredPropertiesInResponse [boolean]', 'allows required properties in response bodies, default false').description("Confirms the swagger spec and mock are compatible with each other.\n\nBasic Usage:\nThe <swagger> and <mock> arguments should paths to the json files or urls to the json files.\n\nSupported Mock Formats:\nPact\n\nPact Broker:\nFor providers using the pact broker the <mock> argument should be the url to the root of the\npact broker and the provider name should be passed using the --provider option. This will\nautomatically find the latest versions of the consumer pact file(s) uploaded to the broker for\nthe specified provider name. The <swagger> argument should be the path or url to the swagger\njson file. Optionally, pass a --tag option alongside a --provider option to filter the retrieved\npacts from the broker by Pact Broker version tags.\n\nIf the pact broker has basic auth enabled, pass a --user option with username and password joined by a colon\n(i.e. THE_USERNAME:THE_PASSWORD) to access the pact broker resources.").action( /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(swagger, mock, options) {
    var swaggerMockValidator, result;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          _context.prev = 0;
          swaggerMockValidator = SwaggerMockValidatorFactory.create(options.user);
          _context.next = 4;
