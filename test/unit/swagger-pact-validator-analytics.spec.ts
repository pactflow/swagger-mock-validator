...
} from './support/swagger-pact-validator-loader';

declare function expect(actual: any): CustomMatchers;

describe('swagger-pact-validator analytics', () => {
    ...
    const invokeValidation = (options: SwaggerPactValidatorOptions) => {
    ...
    const invokeValidationWithUrls = (pactFile?: Pact, swaggerFile?: Swagger): Promise<ValidationSuccess> => {
        ...
        return invokeValidation({
            pactPathOrUrl: 'http://domain.com/pact.json',
            swaggerPathOrUrl: 'http://domain.com/swagger.json'
        });
    };
    ...
    const invokeValidationWithPactBroker = (
        ...
        return invokeValidation({
            pactPathOrUrl: 'http://pact-broker.com',
    ...
    it('should fail with the original error when validation fails and analytics has an error', willResolve(() => {
        ...
        return expectToReject(result).then((error) => {
            expect(error).toEqual(new Error(
                'Pact file "http://domain.com/pact.json" is not compatible ' +
    ...
    it('should send the source', willResolve(() =>
        invokeValidationWithUrls().then(() => {
            expect(getPostBody().source).toBe('swagger-pact-validator');
        })
    ));
});
