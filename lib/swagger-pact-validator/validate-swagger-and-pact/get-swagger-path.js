...
const getParameterDefinition = (interactionContext, pathSegmentContext) => {
    ...
    const pathParamtersWithInPath = _.filter(pathParameters, {in: 'path'});
    const matchingPathParameterIndex = _.findIndex(pathParamtersWithInPath, matchesName);

    if (matchingPathParameterIndex > -1) {
        return pathSegmentContext
            .pushSwaggerLocation('parameters')
            .pushSwaggerLocationArrayIndex(matchingPathParameterIndex)
            .setSwaggerValue(pathParamtersWithInPath[matchingPathParameterIndex]);
    }
...
const getValidator = (options) => {
    ...
    if (options.swaggerPathNameSegment.isParameter) {
        const parameterDefinition = getParameterDefinition(options.interactionContext, options.pathSegmentContext);
...
const doPactAndSwaggerPathSegmentsMatch = (options) => {
    if (!options.swaggerPathNameSegment || !options.pactRequestPathSegment) {
        return {match: false};
    }

    const validator = getValidator(options);

    return validator(options.pathSegmentContext);
};
            ...
            const results = _(swaggerPath.parsedValue.nameSegments.parsedValue)
                ...
                .map((pathSegment) => doPactAndSwaggerPathSegmentsMatch({
                    ...
                    pathSegmentContext: requestPathContext
                        .setPactValue(_.get(pathSegment.pactRequestPathSegment, 'rawValue'))
                        .pushSwaggerLocation(swaggerPath.parsedValue.name.rawValue)
                        .setSwaggerValue(_.get(pathSegment.swaggerPathNameSegment, 'rawValue'))
