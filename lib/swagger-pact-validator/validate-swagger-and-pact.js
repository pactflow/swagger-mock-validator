...
const validatePactInteraction = (interactionContext, interaction, swagger) =>
    _.concat(
        ...
        validatePactRequestBody(interactionContext, interaction, swagger)
    );

module.exports = (context, pact, swagger) => {
    const validationResults = _.map(pact.parsedValue.interactions.parsedValue, (interaction, index) =>
        validatePactInteraction(
            context
    ...
    const results = {
        errors: _(validationResults)
            .flatten()
            .filter((res) => res.type === 'error')
            .value(),
        warnings: _(validationResults)
            .flatten()
