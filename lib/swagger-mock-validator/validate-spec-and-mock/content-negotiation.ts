...
const subtype = (t: string): string | undefined => {
    const [_type, subtype] = t.split(TYPE_SUBTYPE_SEPARATOR);
...
export function findMatchingType(requestType: string, responseTypes: string[]): string | undefined {
    ...
    for (let a of accept) {
        let matchExactly = (t: string): boolean => t === a;
        let index = available.findIndex(matchExactly);
        if (index >= 0) {
    ...
    for (let a of accept) {
        let matchTypesAndSubtypes = (t: string): boolean => type(t) === type(a) && subtype(t) === subtype(a);
        let index = available.findIndex(matchTypesAndSubtypes);
        if (index >= 0) {
            return responseTypes[index];
        }
    }
    ...
    for (let a of accept) {
        let matchSubtype = (t: string): boolean => subtype(t) === WILDCARD && type(t) === type(a);
        let index = available.findIndex(matchSubtype);
        if (index >= 0) {
            return responseTypes[index];
        }
    }
    ...
    for (let a of accept) {
        let matchSubtype = (t: string): boolean => subtype(a) === WILDCARD && type(t) === type(a);
        let index = available.findIndex(matchSubtype);
        if (index >= 0) {
            return responseTypes[index];
        }
